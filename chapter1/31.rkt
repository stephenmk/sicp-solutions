...
;; change the initial "result" value from 0, the additive identity, to
;; 1, the multiplicative identity. Otherwise, the result will always
;; be 0.
...
(define (estimate-pi precision)
  ...
  (define (denominator-term x)
    (if (= 0 (remainder x 2))
        ...
        x))
  (* 4
     (/ (product numerator-term 2 increment precision)
        (product denominator-term 2 increment precision))))

(estimate-pi 1000) ; => 3.1400

;;; Solution Part B
;; Again adjust the book's recursive definition of sum to use the *
;; operator instead of the + operator. Use the multiplicative identity
;; instead of the additive identity as well.
(define (product term a next b)
  (define multiplicative-identity 1)
  (if (> a b)
...
;; Tested and works as expected with estimate-pi and factorial
