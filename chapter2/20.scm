...
notation "dotted-tail notation".  In a procedure definition, a
...
(define (same-parity x . y)
  (let ((parity (remainder x 2))) ; 0=even, 1=odd
    (define (check-parity z)
      (cond ((null? z)
             '())
            ((not (= parity (remainder (car z) 2)))
             (check-parity (cdr z)))
            (else
             (append (list (car z)) (check-parity (cdr z))))))
    (append (list x) (check-parity y))))
